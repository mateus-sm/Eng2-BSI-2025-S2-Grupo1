...
async function carregarDadosIniciais() {
    try {
        ...
        for (const m of membrosData) {
            const respUsuario = await fetch(`${API_USUARIOS}/${m.usuario.id}`)
            if (!respUsuario.ok) throw new Error(`Erro ao carregar usuário ${m.usuario.id}`)
            const usuarioData = await respUsuario.json()

            const nome = usuarioData.nome || `Membro ${m.usuario.id}`
            membros[m.usuario.id] = nome
        ...
        for (const a of admData) {
            const respUsuario = await fetch(`${API_USUARIOS}/${a.usuario.id}`)
            if (!respUsuario.ok) throw new Error(`Erro ao carregar usuário ${a.usuario.id}`)
            const usuarioData = await respUsuario.json()

            const nome = usuarioData.nome || `Admin ${a.id}`
            administradores[a.id] = nome
    ...
    todasAsAtribuicoes.forEach((attr) => {
        ...
        tr.innerHTML = `
          ...
          <td class="text-center">
            <button class="btn btn-sm" onclick="editarAtribuicao(${attr.id}, ${attr.id_membro}, ${attr.id_conquista}, ${attr.id_admin}, '${(attr.data || "").replace(/'/g, "\\'")}', '${(attr.observacao || "").replace(/'/g, "\\'")}')" title="Editar">
